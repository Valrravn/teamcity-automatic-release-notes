...
object RetrieveIssues : BuildType({
    ...
    steps {
        ...
        script {
            name = "Get ToC and Previous Versions from GitHub"
        ...
        python {
            name = "Previous versions: Write a new entry"
        ...
        script {
            ...
            scriptContent = """
                ...
                # replace tocModified with a similar prevVersionsFileModified
                if [ "${'$'}tocModified" == true ]; then
                    curl -s -X PUT -H "Authorization: Bearer ${'$'}GITHUB_TOKEN" \
                    -H "Content-Type: application/json" \
                    -d "{\"message\": \"Commit message\", \"content\": \"${'$'}PREV_CONTENT\", \"branch\": \"auto-release-notes\", \"sha\": \"${'$'}PREV_SHA\"}" \
                    "${'$'}GH_API/contents/topics/previous-releases-downloads.md"
                    
                    else
                    echo "The previous-releases-downloads.md file already contains an matching entry"
                fi
            """.trimIndent()
        }
        script {
