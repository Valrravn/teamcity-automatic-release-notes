...
object RetrieveIssues : BuildType({
    ...
    params {
        ...
        checkbox("env.prevRelModified", "true", display = ParameterDisplay.HIDDEN,
        ...
        checkbox("env.tocModified", "true", display = ParameterDisplay.HIDDEN,
                  checked = "true", unchecked = "false")
    ...
    steps {
        csharpScript {
            name = "C# — Process versions and setup parameters"
            id = "csharpScript"
            content = """
                ...
                Console.WriteLine("Release date: " + r_date);
                ...
                string rnTocLine; // a link to insert to ToC
                ...
                Console.WriteLine("##teamcity[setParameter name='release-date' value='" + r_date + "']");
        ...
        script {
            name = "CLI — Obtain remote versions of files to modify"
            ...
            scriptContent = """
                ...
                curl -H "Accept: application/vnd.github.v3.raw" \
                     -o previous-releases-downloads.md \
                     -L "https://raw.githubusercontent.com/JetBrains/teamcity-documentation/${'$'}repo_branch/topics/previous-releases-downloads.md"
            """.trimIndent()
        }
        python {
            name = "Py — Insert a new ToC link to Release Notes"
            id = "Edit_ToC"
            command = script {
                content = """
                    import re
                    ...
                    def insert_after_entry(search_term, new_entry):
                        ...
                        if not found:
                            ...
                            print(f"##teamcity[setParameter name='env.tocModified' value='false']")
                            return  # Exit without modifying the file
                    
                        if already_exists:
                            print(f"ToC was not modified '{new_entry}' already exists after '{search_term}'.")
                            print(f"##teamcity[setParameter name='env.tocModified' value='false']")
                        else:
        ...
        python {
            name = "Py — Write a new entry to the versions history log"
            id = "Previous_versions_Write_a_new_entry"
            command = script {
                content = """
                    import re
                    
                    FILE_PATH = "previous-releases-downloads.md"
                    
                    NEW_VERSION_TEMPLATE = ${TQ}Release date: %release-date%<br/>
                    Build %build-number%
                    
                    [Windows installer](https://download.jetbrains.com/teamcity/TeamCity-{version}.exe)<br/>
                    [Archive with bundled Tomcat (any platform)](https://download.jetbrains.com/teamcity/TeamCity-{version}.tar.gz)
                    
                    [Release notes](teamcity-{version_dashed}-release-notes.md)
                    $TQ
                    
                    
                    def extract_versions(content):
                        ${TQ}Extracts all TeamCity version headers from the file and returns them as a sorted list.$TQ
                        version_pattern = re.compile(r"^##(?: Current version:)? TeamCity (\d+\.\d+(?:\.\d+)?)", re.MULTILINE)
                        versions = version_pattern.findall(content)
                        
                        # Sort versions in descending order
                        def version_key(v):
                            parts = list(map(int, v.split(".")))  # Convert "2024.12.3" to [2024, 12, 3]
                            while len(parts) < 3:  # Ensure three parts for consistent sorting
                                parts.append(0)
                            return tuple(parts)
                        
                        sorted_versions = sorted(versions, key=version_key, reverse=True)
                        print(f"Current list of TC versions: {sorted_versions}")
                        return sorted_versions
                    
                    def add_version(new_version):
                        ${TQ}Adds a new version section to the file, updating 'Current version' status as needed.$TQ
                        with open(FILE_PATH, "r", encoding="utf-8") as file:
                            content = file.read()
                        ...
                        if new_version in versions:
                            print(f"Version {new_version} already exists. No changes made.")
                            print(f"##teamcity[setParameter name='env.prevRelModified' value='false']")
                            return
                    
                        new_version_parts = list(map(int, new_version.split(".")))
                        
                        # Determine if the newly added version should be "Current version"
                        is_newest = True
                        for existing_version in versions:
                            existing_parts = list(map(int, existing_version.split(".")))
                            if existing_parts[0] > new_version_parts[0] or (
                                existing_parts[0] == new_version_parts[0] and existing_parts[1] > new_version_parts[1]
                            ):
                                is_newest = False
                                break  # There's a newer major or minor version, so stop checking
                    
                        # Edit the inserted text template
                        new_version_text = NEW_VERSION_TEMPLATE.format(
                            version=new_version, 
                            version_dashed=new_version.replace(".", "-")
                        )
                    
                        new_section = f"## {'Current version: ' if is_newest else ''}TeamCity {new_version}\n\n{new_version_text.strip()}\n\n"
                    
                        # If adding the newest version, remove "Current version:" from the previous one
                        if is_newest and versions:
                            content = re.sub(r"^(##) Current version: ", r"\1 ", content, 1, flags=re.MULTILINE)
                    
                        # Find the correct position to insert the new version
                        insert_pos = 0
                        for match in re.finditer(r"^## TeamCity \d+\.\d+(?:\.\d+)?", content, re.MULTILINE):
                        ...
                        print(f"Version {new_version} added successfully.")
                    
                    add_version("%short-version%")
                """.trimIndent()
            }
        }
        script {
            name = "CLI — Extract the list of RN issues from YT"
            id = "simpleRunner"
            scriptContent = """
        ...
        script {
            name = "CLI — Get the total number of security issues from YT"
            id = "Security_issues"
            scriptContent = """
        ...
        csharpScript {
            name = "C# — Pre-transform (format the YT issues response)"
            id = "Transform"
            content = """
        ...
        csharpScript {
            name = "C# — Cleanse (remove extra data from YT response)"
            id = "Cleanse"
            content = """
        ...
        csharpScript {
            name = "C# — Transform (generate Release Notes in acceptable format)"
            id = "Transform_1"
            content = """
                ...
                try {
                	...
                	mdLines.Add("**Build %build-number%, %release-date%**");
        ...
        script {
            name = "CLI — Create new GitHub branch and commit changes"
            id = "simpleRunner_1"
            scriptContent = """
                ...
                PREV_SHA=${'$'}(curl -s -H "Authorization: Bearer ${'$'}GITHUB_TOKEN" \
                    "${'$'}GH_API/contents/topics/previous-releases-downloads.md?ref=auto-release-notes"| jq -r '.sha')
                    
                echo "Previous versions SHA: ${'$'}PREV_SHA"  
                    
                
                # Encode local file content to Base64
                ENCODED_CONTENT=${'$'}(base64 -i "markdown.md")
                TOC_CONTENT=${'$'}(base64 -i "tc.tree")
                PREV_CONTENT=${'$'}(base64 -i "previous-releases-downloads.md")
                
                # Replace remote file content
                curl -s -X PUT -H "Authorization: Bearer ${'$'}GITHUB_TOKEN" \
                    -H "Content-Type: application/json" \
                    -d "{\"message\": \"Generate Release Notes for %short-version%\", \"content\": \"${'$'}ENCODED_CONTENT\", \"branch\": \"auto-release-notes\", \"sha\": \"${'$'}FILE_SHA\"}" \
                ...
                if [ "${'$'}tocModified" == true ]; then
                    curl -s -X PUT -H "Authorization: Bearer ${'$'}GITHUB_TOKEN" \
                    -H "Content-Type: application/json" \
                    -d "{\"message\": \"Insert a ToC Entry\", \"content\": \"${'$'}TOC_CONTENT\", \"branch\": \"auto-release-notes\", \"sha\": \"${'$'}TOC_SHA\"}" \
                    "${'$'}GH_API/contents/tc.tree"
                    
                    else
                    echo "A link to Release Notes article already exists in ToC"
                fi
                
                
                if [ "${'$'}prevRelModified" == true ]; then
                    curl -s -X PUT -H "Authorization: Bearer ${'$'}GITHUB_TOKEN" \
                    -H "Content-Type: application/json" \
                    -d "{\"message\": \"Modify previous releases downloads file\", \"content\": \"${'$'}PREV_CONTENT\", \"branch\": \"auto-release-notes\", \"sha\": \"${'$'}PREV_SHA\"}" \
                    "${'$'}GH_API/contents/topics/previous-releases-downloads.md"
                    
                    else
                    echo "The previous-releases-downloads.md file already contains a matching entry"
                fi
            """.trimIndent()
        }
        script {
            name = "CLI — Create a pull request"
            id = "test"
            scriptContent = """
